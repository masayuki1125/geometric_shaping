...
def make_BICM_int(N,M,type,EsNodB=False,const=False):
        ...
        if type==1:#1:separated scheme 
            ...
            BICM_int=np.reshape(BICM_int,[int(np.log2(M**(1/2))),-1],order='C')
            BICM_int=np.ravel(BICM_int,order='F')
            finite blocklength coded modulation.pdf
...
K=N//2
M_list=[256]#,2**2**6]
type_list=[3,5]#1:separated scheme 2:Block intlv(No intlv in arikan polar decoder) 3:No intlv(Block intlv in arikan polar decoder) 4:rand intlv
